...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const { name, url, logo, desc, catelogId, sort_order, is_private, is_pinned } = config;
    ...
    const insert = await env.NAV_DB.prepare(`
      INSERT INTO sites (name, url, logo, desc, catelog_id, catelog_name, sort_order, is_private, is_pinned)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(sanitizedName, sanitizedUrl, sanitizedLogo, sanitizedDesc, catelogId, categoryResult.catelog, sortOrderValue, finalIsPrivate, isPinnedValue).run();
