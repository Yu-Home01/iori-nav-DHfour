...
export async function onRequestPut(context) {
  ...
  try {
    ...
    const { name, url, logo, desc, catelog_id, sort_order, is_private, is_pinned } = config;
    ...
    const isPinnedValue = is_pinned === true || is_pinned === 1 || is_pinned === '1' ? 1 : 0;
    ...
    const update = await env.NAV_DB.prepare(`
      UPDATE sites
      SET name = ?, url = ?, logo = ?, desc = ?, catelog_id = ?, catelog_name = ?, sort_order = ?, is_private = ?, is_pinned = ?, update_time = CURRENT_TIMESTAMP
      WHERE id = ?
    `).bind(sanitizedName, sanitizedUrl, sanitizedLogo, sanitizedDesc, catelog_id, catelogName, sortOrderValue, finalIsPrivate, isPinnedValue, id).run();
