...
export async function onRequest(context) {
  ...
  const homeCacheKey = getHomeCacheKey(cacheScope) + ':v3';
  ...
  // 注入天气配置供前端使用
  const weatherCities = [S.weather_city_1, S.weather_city_2].filter(Boolean);
  if (S.weather_enabled && weatherCities.length > 0) {
    const weatherConfigScript = `<script>window.IORI_WEATHER_CONFIG={enabled:true,cities:${JSON.stringify(weatherCities)},size:'${S.weather_card_size || '2x'}',sort:${Number(S.weather_card_sort || 0)}};</script>`;
    html = html.replace('</body>', weatherConfigScript + '</body>');
  }
